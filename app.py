...
def render_page(rows: List[Dict[str, Any]], results=None, error_message: str = "", hit_count: int = 0) -> HTMLResponse:
    results = results or []
 
    startup_html = ""
    if STARTUP_ERROR:
        startup_html = f'<div class="error-box">{html.escape(STARTUP_ERROR)}</div>'# app.py
import os
import html
from datetime import date, timedelta
from typing import List, Dict, Any
...
def result_color_map(results: List[Dict[str, Any]]) -> Dict[str, str]:
    colors = {}
    ond_colors = ["c-od1", "c-od2", "c-od3", "c-od4", "c-od5", "c-od6", "c-od7"]
    normal_ond_index = 0
...
async def submit(request: Request):
    ...
    if action == "load_string":
        ...
        return render_page(rows, hit_count=hit_count)
    ...
    if action == "calculate":
        if STARTUP_ERROR:
            return render_page(rows, [], STARTUP_ERROR, hit_count=hit_count)

        valid_rows = [r for r in rows if r.get("origin") and r.get("destination")]

        if not valid_rows:
            return render_page(rows, [], "Please enter at least one valid itinerary row.", hit_count=hit_count)

        try:
            results = calculate_all(rows)
            return render_page(rows, results, hit_count=hit_count)
        except Exception as e:
            return render_page(rows, [], str(e), hit_count=hit_count)
