...
def build_virtual_legs(sectors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    while i < len(sectors):
        ...
        # Hub transfer elimination only when it is NOT same-airport return.
        # Example:
        # JFK-DXB + DXB-SIN => JFK-SIN
        # LHR-DXB + DXB-LHR => keep sector wise, do NOT make LHR-LHR
        if (
            i + 1 < len(sectors)
            and s["destination"] == HUB
            and sectors[i + 1]["origin"] == HUB
            and s["origin"] != sectors[i + 1]["destination"]
            and s["origin_zone"] != sectors[i + 1]["destination_zone"]
            ):
            n = sectors[i + 1]
